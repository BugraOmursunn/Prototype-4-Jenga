...
public class VehicleCameraController : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.touchCount == 2)
        {
            ...
            if (touchesPrevPosDifference > touchesCurPosDifference)
                smoothFollowSettings.distance += zoomModifier;
            if (touchesPrevPosDifference < touchesCurPosDifference)
                smoothFollowSettings.distance -= zoomModifier;

        }

        smoothFollowSettings.distance = Mathf.Clamp(smoothFollowSettings.distance, orbitSettings.minDistance, orbitSettings.maxDistance);
    ...
    void DoSmoothFollow()
    {
        ...
        smoothFollowSettings.distance = Mathf.Clamp(smoothFollowSettings.distance, orbitSettings.minDistance, orbitSettings.maxDistance);
        m_orbitDistance = Mathf.Lerp(m_orbitDistance, smoothFollowSettings.distance, orbitSettings.distanceDamping * Time.deltaTime);
        //

        m_transform.position -= currentRotation * Vector3.forward * m_orbitDistance;
    ...
    private float m_orbitDistanceSmooth;

    void InitializeMouseOrbit()
    {
        m_orbitDistance = orbitSettings.distance;
        m_orbitDistanceSmooth = smoothFollowSettings.distance;
